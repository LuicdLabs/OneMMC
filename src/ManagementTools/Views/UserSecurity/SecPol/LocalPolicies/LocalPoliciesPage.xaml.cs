...
public sealed partial class LocalPoliciesPage : Page
{
    ...
    private readonly ILogger<LocalPoliciesPage> _logger;
    public LocalPoliciesViewModel ViewModel { get; }

    public LocalPoliciesPage()
    {
        _logger = App.GetRequiredService<ILogger<LocalPoliciesPage>>();
        ViewModel = App.GetRequiredService<LocalPoliciesViewModel>();

        _logger.LogDebug("[LocalPoliciesPage] Initializing");
        InitializeComponent();
    ...
    {
        _logger.LogDebug("[LocalPoliciesPage] Page loaded");
    ...
    private void PolicyTree_ItemInvoked(TreeView sender, TreeViewItemInvokedEventArgs e)
    {
        ...
        {
            _logger.LogDebug("[LocalPoliciesPage] Category selected: {CategoryDisplayName}", category.DisplayName);
    ...
    private async void EditButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        _logger.LogDebug("[LocalPoliciesPage] Edit clicked: {PolicyKey}", ViewModel.SelectedPolicy.Definition.Key);
    ...
    private async void PoliciesList_DoubleTapped(object sender, DoubleTappedRoutedEventArgs e)
    {
        ...
        _logger.LogDebug("[LocalPoliciesPage] Policy double-tapped: {PolicyKey}", ViewModel.SelectedPolicy.Definition.Key);
        await OpenEditorDialogAsync(ViewModel.SelectedPolicy);
    }

    private void RefreshButton_Click(object sender, RoutedEventArgs e)
    {
        _logger.LogDebug("[LocalPoliciesPage] Refresh clicked");
    ...
    {
        _logger.LogDebug("[LocalPoliciesPage] Export clicked");
        if (ViewModel.CurrentPolicies.Count == 0) return;

        var hwnd = WinRT.Interop.WindowNative.GetWindowHandle(App.MainWindowInstance);
        var path = await App.GetRequiredService<ManagementTools.Core.Abstractions.Services.IFileDialogService>().SaveFileAsync(
            hwnd,
        ...
        _logger.LogInformation("[LocalPoliciesPage] Exported {PolicyCount} policies to {ExportPath}", ViewModel.CurrentPolicies.Count, path);
    }

    private async System.Threading.Tasks.Task OpenEditorDialogAsync(SecurityPolicyValue policy)
    {
        _logger.LogDebug("[LocalPoliciesPage] Opening editor for: {PolicyKey}", policy.Definition.Key);
        ...
        {
            _logger.LogDebug("[LocalPoliciesPage] Saving edited policy: {PolicyKey}", dialog.EditedValue.Definition.Key);
