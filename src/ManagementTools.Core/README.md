# ManagementTools.Core

`ManagementTools.Core` is the Windows-native application layer for ManagementTools.
...
- The only Core DI entrypoint is `AddManagementToolsCore(this IServiceCollection services)`.
- Core registration is explicit. Do not add reflection-based auto scanning back.
- Feature modules stay internal and are called by `AddManagementToolsCore(...)`.
...
- `ManagementTools.Core.ViewModels.*`
- `ManagementTools.Core.Models.*`
- `ManagementTools.Core.Services.*`
- mirror namespaces
