...
namespace OneMMC.Views.PerfMon;
...
/// <summary>
    ...
    private async Task<string?> OpenFileAsync(string filter, string? title = null)
    {
        if (App.MainWindowInstance == null) return null;
        var hwnd = WindowNative.GetWindowHandle(App.MainWindowInstance);
        return await App.GetRequiredService<OneMMC.Core.Abstractions.Services.IFileDialogService>().OpenFileAsync(hwnd, filter, title);
    }
